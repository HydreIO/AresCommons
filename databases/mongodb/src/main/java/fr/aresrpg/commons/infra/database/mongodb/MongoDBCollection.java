...
public class MongoDBCollection<T> implements Collection<T> {
	...
	public void putOrUpdate(Filter filter, T t) {
		if (filter == null) put(t);
		else update(filter, t);
	}
	...
	public T[] find(Filter filter, int limit) {
		try {
			...
			if (filter == null) cursor = collection.find().iterator();
			else cursor = collection.find(toMongoDBFilter(filter)).iterator();
			int i;
