...
public class Serializer {
	...
	public  <T> void serialize(OutputStream stream , T value , ParametrizedClass<T> clazz) throws IOException {
		...
		if(adapter != null)
			adapter.adaptTo(stream , value , context , format);
		else
			format.write(stream , value , context);//If format call serialize deepSerialize
	}
	...
	protected <T> void serializeObject(OutputStream stream , T value , ParametrizedClass<T> clazz) throws IOException {
		...
		if(adapter != null)
			...
			format.writeObject(stream , value , context);//If format call serialize deepSerialize
	}
	...
	public <T> Adapter<T> getAdapter(ParametrizedClass<T> clazz){
		...
		for(Adapter adapter : adapters)
			if(clazz.equals(adapter.getType()))
				return adapter;
		return null;
	}
